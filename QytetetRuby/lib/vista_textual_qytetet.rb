...
module InterfazTextualQytetet
  class VistaTextualQytetet
    ...
    def elegir_quiero_comprar
      ...
      puts("¿Quieres comprar este título de propiedad?")
      puts("Sí --> S")
      puts("No --> N")
      ...
      while incorrecto do
        ...
        if respuesta.eql?("S") or respuesta.eql?("s") then
          elegir = true
        elsif respuesta.eql?("N") or respuesta.eql?("n") then
          elegir = false
        else
          puts("Respuesta no válida: elija Sí(S) o No(N)")
          incorrecto = true
        end
      end
